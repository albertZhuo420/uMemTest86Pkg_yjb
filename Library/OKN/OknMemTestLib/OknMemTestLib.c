...
EFI_STATUS OknMT_ReadSpdToJson(IN OKN_MEMORY_TEST_PROTOCOL *pProto, IN OUT cJSON *pJsTree)
{
  ...
  Socket  = cJSON_GetObjectItemCaseSensitive(pJsTree, "Socket");
  Channel = cJSON_GetObjectItemCaseSensitive(pJsTree, "Channel");
  Dimm    = cJSON_GetObjectItemCaseSensitive(pJsTree, "Dimm");
  if (!cJSON_IsNumber(Socket) || !cJSON_IsNumber(Channel) || !cJSON_IsNumber(Dimm)) {
